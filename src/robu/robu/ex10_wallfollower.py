...
class WallFollowerer(rclpy.Node):
    def __init__(self):
        ...
        self.wallfollower_state = WallFollowerer.WF_STATE_DETECTWALL
    ...
    def scan_callback(self, msg):
        ...
        print("ld: %.2f m" % self.left_dist,
              "lfd: %.2f m" % self.left_front_dist,
              "fd: %.2f m" % self.front_dist,
              "rfd: %.2f m" % self.right_front_dist,
              "rd: %.2f m" % self.right_dist,
              "rrd: %.2f m" % self.rear_dist)

def main(args=None):
    rclpy.init(args=args)
    wallfollower = WallFollowerer()
